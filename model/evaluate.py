...
def infer(dmm, dataset, mask):
    ...
    """
    dmm.resetDataset(dataset,mask,quiet=True)
    assert len(dataset.shape)==3,'Expecting 3D tensor for data' 
    assert dataset.shape[2]==dmm.params['dim_observations'],'Data dim. not matching'
    return dmm.posterior_inference(idx=np.arange(dataset.shape[0]))

def evaluateBound(dmm, dataset, mask, batch_size,S=2, normalization = 'frame', additional={}):
    """ Evaluate ELBO """
    bound = 0
    ...
    for bnum,st_idx in enumerate(range(0,N,batch_size)):
        ...
        for s in range(S):
            if s>0 and s%500==0:
                dmm._p('Done '+str(s))
            batch_vec= dmm.evaluate(idx=idx_data)
            M = mask[idx_data]
            if np.any(np.isnan(batch_vec)) or np.any(np.isinf(batch_vec)):
                dmm._p('NaN detected during evaluation. Ignoring this sample')
                maxS -=1
                continue
            else:
    ...
    end_time   = time.time()
    dmm._p(('(Evaluate) Validation Bound: %.4f [Took %.4f seconds], TSBN Bound: %.4f')%(bound,end_time-start_time,tsbn_bound))
    additional['tsbn_bound'] = tsbn_bound
    return bound


def impSamplingNLL(dmm, dataset, mask, batch_size, S = 2, normalization = 'frame'):
    ...
    for bnum,st_idx in enumerate(range(0,N,batch_size)):
        ...
        for s in range(S):
            if s>0 and s%500==0:
                dmm._p('Done '+str(s))
            batch_vec = dmm.likelihood(idx=idx_data)
            if np.any(np.isnan(batch_vec)) or np.any(np.isinf(batch_vec)):
                dmm._p('NaN detected during evaluation. Ignoring this sample')
                maxS -=1
                continue
        ...
        ll  += dmm.meanSumExp(np.concatenate(lllist,axis=1), axis=1).sum()
    if normalization=='frame':
        ll /= float(mask.sum())
    elif normalization=='sequence':
        ll /= float(N)
        ...
        assert False,'Invalid normalization specified'
    end_time   = time.time()
    dmm._p(('(Evaluate w/ Imp. Sampling) Validation LL: %.4f [Took %.4f seconds]')%(ll,end_time-start_time))
    return ll


def sampleGaussian(dmm,mu,logcov):
        return mu + np.random.randn(*mu.shape)*np.exp(0.5*logcov)

def sample(dmm, nsamples=100, T=10, additional = {}):
    """
                                  Sample from Generative Model
    """
    assert T>1, 'Sample atleast 2 timesteps'
    #Initial sample
    z      = np.random.randn(nsamples,1,dmm.params['dim_stochastic']).astype(config.floatX)
    all_zs = [np.copy(z)]
    additional['mu']     = []
    additional['logcov'] = []
    for t in range(T-1):
        mu,logcov = dmm.transition_fxn(z)
        z           = dmm.sampleGaussian(mu,logcov).astype(config.floatX)
    ...
    return dmm.emission_fxn(zvec), zvec
